...
class SimpleEmbedding(BasePose):
    ...
    def forward_test(self, img, img_metas, return_heatmap=False, **kwargs):
        ...
        for idx, s in enumerate(sorted(test_scale_factor, reverse=True)):
            ...
            # t2 = time.time()
            # print("featureTime:",t2-t1)
            if self.with_neck:
                features = self.neck(features)
            if self.with_keypoint:
                outputs = self.keypoint_head(features)
                # print("headTime:",time.time()-t2)
            ...
            if self.test_cfg.get('flip_test', True):
                ...
                heatmaps_flipped = outputs_flipped
        ...
        # print('flipTime:', time.time() - t2)

        # t2 = time.time()
        # perform grouping
        grouped, scores = self.parser.parse2(aggregated_heatmaps,

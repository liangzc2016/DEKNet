...
class dw3x3(nn.Module):
    def __init__(self, in_ch, out_ch):
        super(dw3x3, self).__init__()
        ...
        # self.point_conv = nn.Sequential(nn.Conv2d(
        #     in_channels=in_ch,
        #     out_channels=out_ch,
        #     kernel_size=1,
        #     stride=1,
...
class HRModule(nn.Module):
    ...
    def __init__(self,
        ...
        # self.dw3x3 = self.make_dw3x3(num_channels)

    @staticmethod
    def make_dw3x3(num_channels):
        dwRes=[]
        for i in range(len(num_channels)):
            dwRes.append(dw3x3(num_channels[i],num_channels[i]).to(device))
            # dwRes.append(nn.Sequential(
            #     nn.Conv2d(num_channels[i],num_channels[i],kernel_size=3,stride=1,groups=num_channels[i],padding=1),
            #     # build_norm_layer(
            #     #     dict(type='BN'),
            #     #     num_channels[i] )[1],
            #     nn.BatchNorm2d(num_channels[i]),
            #     nn.ReLU(),
            #     nn.Conv2d(num_channels[i], num_channels[i], kernel_size=1, padding=0, stride=1,bias=False),
            #     nn.BatchNorm2d(num_channels[i]),

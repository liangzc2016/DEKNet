...
class SoftWingLoss(nn.Module):
    ...
    def criterion(self, pred, target):
        ...
        Args:
            ...
            target (torch.Tensor[N, K, D]): Target regression.
        """
        N ,k , h,w= pred[0].shape[0],pred[0].shape[1],pred[0].shape[2],pred[0].shape[3]
        # pred = pred[0].view(N,-1,2)
        pred = pred[0]
        target = target[0].view_as(pred)
        delta = (target - pred).abs()
        losses = torch.where(
            delta < self.omega1, delta,
            self.omega2 * torch.log(1.0 + delta / self.epsilon) + self.B)
        # losses = losses.view(N,k,h,w)
        losses = losses.mean(dim=3).mean(dim=2).mean(dim=1)
        return losses
        # return torch.mean(torch.sum(losses, dim=[1, 2]), dim=0)

...
class CsrnetMSELoss(nn.Module):
    ...
    def forward(self, output, target):
        """Forward function."""
        assert len(output) == 1, "error, the dim of loss input is {}".format(len(output))
        assert len(target) == 1, "error, the dim of loss input is {}".format(len(output))
        output = output[0]
        target = target[0]
        batch_size = output.size(0)
        num_joints = output.size(1)
